...
public:
    ...
    void A()
    {
        for (int i = 0; i < loop_num; i++)
        {
            ...
            cv.wait(ulk, [&] {cout << "A is running!" << endl; return index%3 == 0;});
            cout << "A";
    ...
    void B()
    {
        ...
        {
            unique_lock<mutex> ulk(mtx);
            cv.wait(ulk, [&] {cout << "B is running!" << endl;return index%3 == 1;});
            cout << "B";
            this_thread::sleep_for(chrono::milliseconds(100));
            index ++;
            cv.notify_one();
    ...
    void C()
    {
        ...
        {
            unique_lock<mutex> ulk(mtx);
            cv.wait(ulk, [&] {cout << "C is running!" << endl;return index%3 == 2;});
            cout << "C";
            this_thread::sleep_for(chrono::milliseconds(100));
            index ++;
            cv.notify_one();
